...
## 📢 最新更新
### V0.0.9 版本内容 (2026-01-19)
#### ✅ Bug修复
1. 修复了在搜索结果页面点击文件名直接下载的问题
   - 移除了直接点击文件名下载的功能，避免了用户操作混淆
   - 修复了在搜索结果页面点击分享按钮后，分享链接包含文件名的问题
   - 修复搜索文件不显示图标，搜索结果页面的交互逻辑，提升了用户体验 
...
2. **搜索结果页面优化**
   - 为搜索结果添加了下载按钮和分享按钮
   - 移除了直接点击文件名下载的功能，统一通过下载按钮操作
   - 优化了搜索结果的显示布局，提升用户体验

3. **页脚信息可配置化**
   - 在配置文件中添加了页脚信息配置选项
   - 支持自定义版权信息和浏览器建议内容
   - 配置文件中新增 `footer_text` 和 `browser_tips` 字段
   - 保持向后兼容性，未配置时使用默认值

#### 🔧 配置增强
1. **配置文件优化**
   - 新增页脚信息配置字段
   - 支持自定义服务器名称、版权信息和浏览器建议
   - 配置文件结构更加灵活，便于后续扩展
...
#### 📱 UI/UX优化
1. **页脚年份更新**
   - 将页脚年份从2024-2025更新为2025-2026
   - 保持版权信息的时效性

2. **搜索结果交互优化**
   - 统一的操作入口：所有下载操作通过下载按钮进行
   - 清晰的视觉反馈：下载和分享按钮样式鲜明
   - 增强的用户体验：减少用户操作混淆
...
| 日期       | 版本     | 主要更新内容                                                                 |
|------------|----------|------------------------------------------------------------------------------|
| 2026-01-14 | **v0.0.9** | 实现无需登录的下载和分享功能，优化搜索结果页面，添加页脚信息可配置化，更新页脚年份为2025-2026，优化配置加载逻辑 |
