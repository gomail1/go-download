...
### V0.0.7 版本内容 (2025-12-28)

#### 🔐 权限管理优化
- ✅ **二级管理员权限调整**：优化二级管理员权限结构
- ✅ **普通用户管理**：二级管理员只能查看和管理普通用户
- ✅ **文件操作权限**：二级管理员可以执行所有文件/目录操作（删除、移动、复制、创建目录等）
- ✅ **界面访问权限**：二级管理员可以访问所有界面（包括热力图）
- ✅ **安全限制**：二级管理员不能查看或修改管理员/二级管理员账号

#### 🌐 多协议下载集成
- ✅ **多协议支持**：暂时仅支持 HTTP/HTTPS 协议，FTP/SFTP、BitTorrent 和流媒体协议 关闭维护修复BUG。
- ✅ **多线程加速**：支持多线程下载，提高下载速度
- ✅ **断点续传**：支持断点续传功能，保障下载可靠性
- ✅ **随机端口生成**：为 BitTorrent 添加随机端口生成功能，防止 ISP 封锁
- ✅ **下载任务管理**：新增下载任务列表和任务详情页面
- ✅ **子用户审核机制**：默认创建 `download-user` 子用户，所有下载文件自动进入待审核队列
...
#### 📱 UI/UX 优化
- ✅ **移动端导航优化**：实现移动端水平滚动导航菜单
- ✅ **按钮样式统一**：修复按钮大小不一致问题，统一按钮样式
- ✅ **待审核数量提示**：在导航菜单中添加待审核文件数量的红色提示标记
- ✅ **审核页面优化**：修复文件审核页面的按钮布局问题
- ✅ **下载任务页面优化**：修复下载任务列表页面的显示问题，移除冗余按钮
- ✅ **任务详情页实现**：新增下载任务详情页面，显示任务状态、进度、速度等信息
- ✅ **ICO-LOGO设计**：完成网站图标(ICO)设计和集成
- ✅ **用户登出优化**：实现用户登出后自动返回首页功能
- ✅ **上传文件路径选择**：上传文件时采用树形结构进行文件夹路径选取

#### 🛠️ 技术升级
- ✅ **Go 版本升级**：从 Go 1.21.0 升级到 1.25.0
- ✅ **配置文件优化**：优化配置文件结构，统一管理
- ✅ **日志优化**：移除调试信息，防止日志 spam
- ✅ **包名优化**：优化包名结构，避免冲突

#### 📊 API接口优化
- ✅ **API鉴权功能**：为API接口添加鉴权机制，仅管理员和二级管理员可以访问
- ✅ **访问方式支持**：支持网页界面自动会话验证和第三方工具Authorization头验证
- ✅ **接口保护**：保护统计接口和分享计数接口，防止未授权访问
- ✅ **暴力破解防护**：实现基于IP的失败登录尝试限制和封禁机制
- ✅ **IPv6支持**：修复IPv6地址解析问题，确保防护机制对IPv6地址同样有效

##### API鉴权机制说明
1. **网页界面访问**：
   - 自动通过浏览器会话Cookie验证用户身份
   - 仅管理员和二级管理员有权限访问API接口
   - 无需额外配置，登录后即可正常使用界面中的API功能

2. **第三方工具访问**：
   - 通过HTTP请求头的Authorization字段进行验证
   - 直接在Authorization头中传递管理员/二级管理员密码
   - 示例：`curl -H "Authorization: admin123" http://IP:9980/api/stats`

##### 暴力破解防护机制
为了防止通过第三方工具进行密码暴力破解攻击，系统实现了完善的IP级防护机制：

- **失败尝试限制**：单个IP默认最多允许5次失败登录尝试，超过阈值将触发封禁
- **IP封禁策略**：触发阈值后，IP将被自动封禁5分钟，在此期间无法进行API访问
- **自动记录清理**：系统每10分钟自动清理过期的失败记录，保持资源高效利用
- **IPv6完整支持**：正确解析和处理IPv6地址格式（如`::1`），确保防护机制对所有IP类型有效
- **并发安全设计**：使用互斥锁保护失败尝试记录，确保在高并发情况下计数准确
- **智能封禁管理**：封禁期结束后保留失败尝试记录，防止攻击者在封禁结束后立即继续尝试

**防护效果说明**：
这种防护机制能有效保障API访问安全，系统默认的防护策略在确保正常用户使用便捷性的同时，提供了强大的安全保障。

##### API接口列表
| 接口路径 | 方法 | 功能描述 | 权限要求 |
|---------|------|---------|----------|
| `/api/stats` | GET | 获取系统统计信息 | 管理员/二级管理员 |
| `/api/increment-share` | POST | 增加文件分享计数 | 管理员/二级管理员 |
...
### 历史版本概述
- **v0.0.6**：实现移动端适配优化，包括6级媒体查询断点、触摸友好交互、响应式表格设计等
...
- **多协议集成**: 实现多协议下载功能，暂时仅支持 HTTP/HTTPS
