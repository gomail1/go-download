...
12. **文件统计(stats.json)历史重建（09-06 补充）**：首页「累计下载」、文件列表每文件次数、热力图读的都是 `config/stats.json`——旧版每个 Range 分片传输都会 **+1 download_count、追加热力点，并按整文件大小累加 total_bandwidth**，导致比 IP 统计更严重的污染（真实服务器实证：累计 12535 次/30TB → 日志真值 1048 次/43.75GB，次数虚高 12 倍、流量虚高约 690 倍）。已修复：实时路径不再分片累加（含热力点随 #11 一并修正），并新增**按历史日志一键重建 stats.json**——首页「🧰 自检修复」弹窗升级为双卡（IP 统计 + 文件统计/热力图，各自对账与执行修复）；启动参数 **`-rebuild-stats`**；配置项 **`server.rebuild_stats_on_boot: true`**。重建以 (IP+文件) 60s 合并口径从全部日志（含 `.gz`）重算每文件下载次数/带宽/热力点，**分享计数与上传时间保留原值**，自动备份 `stats.json.bak.<时间戳>`，可重复执行
...
5. **下载统计自检修复（V1.3.0 新增，面向升级部署）**
   ...
   - **文件统计重建细节**：自动备份 `stats.json.bak.<时间戳>`；分享计数(ShareCount)与上传时间与分片 bug 无关，重建时保留原值；download_count / total_bandwidth / download 热力点全部按 (IP+文件) 60s 口径从日志重算（带宽按每片实际传输字节累加，不重复）
