...
#### 新增功能
- ✅ **每日上传限制功能**：为普通用户添加了每日上传限制，减少了磁盘I/O操作
- ✅ **页面内消息提醒**：将错误弹窗改为页面内消息，提供更好的用户体验

#### 开发过程中的优化
- ✅ **优化上传过程**：优化了每日上传限制功能，减少了磁盘I/O操作
- ✅ **优化上传限制显示**：确保超出上传限制时正确显示失败信息
- ✅ **优化后端错误处理**：根据请求类型返回适当的响应，AJAX请求返回JSON，普通请求返回HTML
- ✅ **优化前端错误处理**：上传完成后检查响应内容，根据响应类型和内容决定显示成功还是失败
- ✅ **优化用户限制逻辑**：只有管理员账户不受限制，其他所有登录用户都受到上传，审核限制
- ✅ **移除测试用户**：删除了测试用户相关的代码，简化了用户管理
- ✅ **优化界面布局**：管理员界面移除了创建目录功能，改到了文件列表界面
